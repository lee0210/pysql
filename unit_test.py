...
image_write = pysql.table('images_unit_test', columns)
for i in range(0, 5):
...
image_iter = pysql.table('images_unit_test', columns)
image_iter2 = pysql.table('images_unit_test', columns2)
...
image_update = pysql.table('images_unit_test', columns, key)
image_update.id = image_write.id
image_update.chain()
image_update.name = 'test update'
image_update.update()

image_chain = pysql.table('images_unit_test', columns)
image_chain.id = image_write.id
image_chain.chain(key)
print 'Test chain', image_chain.get_dict()

image_delete = pysql.table('images_unit_test', columns)
...
lock_image = pysql.lock_table('images_unit_test', columns, key)
for i in range(0, 10):
    image_write.id = uuid.uuid1().get_hex()
